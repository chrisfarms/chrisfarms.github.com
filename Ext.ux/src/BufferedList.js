...
Ext.ux.BufferedList = Ext.extend(Ext.List, {
    ...
    // filter the collection (the BufferedList version of collectData like collectData)
    filterData: function(records){
        return records;
    },
    ...
    refresh: function() {
        ...
        this.bottomProxy.setHeight(this.records.length * this.maxItemHeight);
    ...
    buildItemHtml: function(firstItem,lastItem) {
        ...
        for ( i = firstItem; i <= lastItem; i++ ) {
            ...
            if ( grpHeads ) {
                groupId = this.store.getGroupString(record);
    ...
    replaceItemList: function(firstNew,nItems) {
        if(!this.records)
            this.records = []; //TODO: where does this _really_ belong?
        var sc = this.records.length;
        if ( firstNew >= sc ) {
            return 0;
        }
    ...
    initGroupIndexMap: function() {
        ...
        var i, 
            ...
            prevGroup = '',
            sc = this.records.length;

        // build temporary map of group string to store index from store records
        for ( i = 0; i < sc; i++ ) {
            key = this.store.getGroupString(this.records[i]).toLowerCase();
    ...
    indexOfRecord: function(rec) {
        ...
        var idx = -1, 
            sc = this.records.length;
        if ( this.grouped ) {
            for ( idx = this.groupStartIndex(this.store.getGroupString(rec)); idx < sc; idx++ ) {
                if ( this.records[idx] === rec ) {
                    break;
                }
            }
        }
        else {
            idx = this.records.indexOf(rec);
        }
        return idx;
    },
    ...
    scrollIntoView: function(record){
        var h = this.maxItemHeight * this.indexOfRecord(record);
        this.scroller.updateBoundary();
        this.scroller.suspendEvents();
        this.scroller.scrollTo({x: 0, y: h});
        this.scroller.resumeEvents();
        this.renderOnScroll();
    },
    ...
    getNode : function(nodeInfo) {
        ...
        if ( Ext.isNumber(nodeInfo) ) {
            return this.all.elements[nodeInfo - this.topItemRendered] || null;
        }
        return Ext.ux.BufferedList.superclass.getNode.call(this, nodeInfo);
    },
    
    getRecord: function(node) {
        return this.records[node.viewIndex];
    },
