...
Ext.ux.IndexedStore = Ext.extend(Ext.data.Store,{
    // generate an index for the passed in array
    ...
    // returns an ObjectIndex for this store
    // the index can be used to make queries on the range
    getIndex: function(){
        return this.getIndexFor(this.getRange);
    },
    
    // directly run a query against this store
    query: function(q){
        return this.getIndex().query(q);
    }
});